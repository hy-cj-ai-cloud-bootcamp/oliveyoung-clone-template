...
올리브영 온라인 쇼핑몰을 간소화하여 구현한 클론 프로젝트입니다.
Docker 기반으로 프론트엔드, 백엔드, AI 챗봇, 데이터베이스를 한 번에 실행할 수 있습니다.
...
| GET | /api/products | 상품 목록 (?category_id=N) |
...
> 중요: 현재 프론트 코드는 `/api/products`, `/api/auth/login`처럼 상대 경로로 API를 호출합니다. 따라서 CloudFront에서 `/api/*` 요청을 ALB로 보내야 합니다. S3만 연결하면 API 요청이 실패합니다.
...
### 아키텍처
```
사용자 질문 → Streamlit → FastAPI → DB에서 상품 데이터 조회
                                         ↓
                                  상품 정보를 프롬프트에 포함
                                         ↓
...
### 구현 방식
- 데이터: RDS에 저장된 상품 데이터를 DB에서 직접 조회
- 이미지: S3에 상품 이미지 업로드 후 DB의 image_url 컬럼 업데이트
- AI: Bedrock InvokeModel로 Claude 직접 호출 (Knowledge Base 불필요)

### 구현 순서
1. Bedrock 모델 접근 활성화 (AWS 콘솔 → Bedrock → Model access)
2. `.env`에 AWS 인증 정보 + `BEDROCK_MODEL_ID` 설정
...
- 실제 올리브영 서비스와 무관하며, 상업적 목적이 아닌 클라우드/AI 실습 목적으로만 사용합니다.
